...
export default function AdminPage(){
  ...
  return <main className="page"><h2>管理</h2><a className="btn" href="/admin/etf-status">ETF 資料健康檢查</a> <a className="btn" href="/reference-etfs">一般 ETF 參考清單</a> <button className="btn" onClick={()=>run('/admin/seed-demo')}>建立 Demo 資料</button> <button className="btn btn-primary" onClick={()=>run('/admin/update-all?dt_range=1')}>更新全部 ETF 最新資料</button><pre className="card" style={{whiteSpace:'pre-wrap'}}>{msg}</pre></main>
}
