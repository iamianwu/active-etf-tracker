...
HEADERS = {"User-Agent": "Mozilla/5.0 AppleWebKit/537.36 Chrome Safari", "Accept": "application/json,text/plain,*/*", "Accept-Language": "zh-TW,zh;q=0.9,en-US;q=0.8"}
...
def pick(row: dict[str, Any], keys: list[str]) -> Any:
    ...
    for k in keys:
        ...
        if kk in norm and row[norm[kk]] not in (None, ""): return row[norm[kk]]
    return None

def find_code(row: dict[str, Any]) -> str | None:
    v = pick(row, ["Code", "證券代號", "有價證券代號", "股票代號", "代號", "SecuritiesCompanyCode", "SecuritiesCode", "CompanyCode", "stock_id"])
    if v is not None:
...
def row_to_quote(row: dict[str, Any], market: str, fallback_trade_date: str | None = None) -> dict[str, Any] | None:
    ...
    if close is None or close <= 0: return None
    change = to_float(pick(row, ["Change", "漲跌價差", "漲跌", "漲跌(+/-)", "漲跌(+/-)價差", "ChangeAmount", "PriceChange"]))
    pct = to_float(pick(row, ["ChangePercent", "漲跌幅", "漲跌幅(%)", "漲跌幅%", "Change%"] ))
    if pct is None and change is not None:
        prev = close - change
        if prev > 0: pct = change / prev * 100.0
...
def ensure_tables() -> None:
    ...
    with get_conn() as conn:
        if conn.postgres:
            for col, typ in {"change":"DOUBLE PRECISION", "volume":"DOUBLE PRECISION", "amount":"DOUBLE PRECISION", "market":"TEXT", "source":"TEXT", "trade_date":"TEXT"}.items():
                conn.execute(f"ALTER TABLE stock_quotes ADD COLUMN IF NOT EXISTS {col} {typ}")
            conn.execute("""CREATE TABLE IF NOT EXISTS stock_price_history(stock_code TEXT NOT NULL, trade_date TEXT NOT NULL, PRIMARY KEY(stock_code, trade_date))""")
            for col, typ in {"stock_name":"TEXT", "open":"DOUBLE PRECISION", "high":"DOUBLE PRECISION", "low":"DOUBLE PRECISION", "close":"DOUBLE PRECISION", "change":"DOUBLE PRECISION", "change_pct":"DOUBLE PRECISION", "volume":"DOUBLE PRECISION", "amount":"DOUBLE PRECISION", "market":"TEXT", "source":"TEXT", "updated_at":"TEXT"}.items():
                conn.execute(f"ALTER TABLE stock_price_history ADD COLUMN IF NOT EXISTS {col} {typ}")
        else:
            rows = conn.execute("PRAGMA table_info(stock_quotes)").fetchall(); cols = {r["name"] for r in rows}
            for col, typ in {"change":"REAL", "volume":"REAL", "amount":"REAL", "market":"TEXT", "source":"TEXT", "trade_date":"TEXT"}.items():
                if col not in cols: conn.execute(f"ALTER TABLE stock_quotes ADD COLUMN {col} {typ}")
            conn.execute("""CREATE TABLE IF NOT EXISTS stock_price_history(stock_code TEXT NOT NULL, trade_date TEXT NOT NULL, PRIMARY KEY(stock_code, trade_date))""")
            rows = conn.execute("PRAGMA table_info(stock_price_history)").fetchall(); cols = {r["name"] for r in rows}
...
def fetch_official_quotes(only_codes: set[str] | None = None, fallback_trade_date: str | None = None) -> dict[str, dict[str, Any]]:
    ...
    for market, url in [("TWSE", TWSE_URL), ("TPEX", TPEX_URL)]:
        ...
        try:
            rows = fetch_array(url); print(f"{market} rows={len(rows)}", flush=True)
        ...
        for row in rows:
            ...
            parsed += 1
            code = q["stock_code"]
            if only_codes is not None and code not in only_codes: continue
            out[code] = q; kept += 1
        ...
        for code, q in quotes.items():
            if conn.postgres:
                ...
                ON CONFLICT (stock_code) DO UPDATE SET
                  stock_name=COALESCE(EXCLUDED.stock_name, stock_quotes.stock_name), price=EXCLUDED.price, change=EXCLUDED.change, change_pct=EXCLUDED.change_pct, volume=EXCLUDED.volume, amount=EXCLUDED.amount, market=EXCLUDED.market, source=EXCLUDED.source, trade_date=EXCLUDED.trade_date, updated_at=EXCLUDED.updated_at
                """, (code, q.get("stock_name"), q.get("price"), q.get("change"), q.get("change_pct"), q.get("volume"), q.get("amount"), q.get("market"), q.get("source"), q.get("trade_date"), now))
                conn.execute("""
                ...
                ON CONFLICT (stock_code, trade_date) DO UPDATE SET
                  stock_name=EXCLUDED.stock_name, open=EXCLUDED.open, high=EXCLUDED.high, low=EXCLUDED.low, close=EXCLUDED.close, change=EXCLUDED.change, change_pct=EXCLUDED.change_pct, volume=EXCLUDED.volume, amount=EXCLUDED.amount, market=EXCLUDED.market, source=EXCLUDED.source, updated_at=EXCLUDED.updated_at
                """, (code, q.get("trade_date"), q.get("stock_name"), q.get("open"), q.get("high"), q.get("low"), q.get("price"), q.get("change"), q.get("change_pct"), q.get("volume"), q.get("amount"), q.get("market"), q.get("source"), now))
            else:
                conn.execute("""INSERT OR REPLACE INTO stock_quotes(stock_code, stock_name, price, change, change_pct, volume, amount, market, source, trade_date, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""", (code, q.get("stock_name"), q.get("price"), q.get("change"), q.get("change_pct"), q.get("volume"), q.get("amount"), q.get("market"), q.get("source"), q.get("trade_date"), now))
                conn.execute("""INSERT OR REPLACE INTO stock_price_history(stock_code, trade_date, stock_name, open, high, low, close, change, change_pct, volume, amount, market, source, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""", (code, q.get("trade_date"), q.get("stock_name"), q.get("open"), q.get("high"), q.get("low"), q.get("price"), q.get("change"), q.get("change_pct"), q.get("volume"), q.get("amount"), q.get("market"), q.get("source"), now))
            saved += 1
            if saved % 50 == 0: print(f"Saved quotes: {saved}/{len(quotes)}", flush=True)
    return saved
