...
def find_code(row: dict[str, Any]) -> str | None:
    v = pick(row, [
        "Code", "證券代號", "有價證券代號", "股票代號", "代號",
        "SecuritiesCompanyCode", "SecuritiesCode", "CompanyCode", "stock_id"
    ])
    if v is not None:
...
def row_to_quote(row: dict[str, Any], market: str) -> dict[str, Any] | None:
    ...
    name = str(pick(row, [
        "Name", "證券名稱", "有價證券名稱", "股票名稱", "名稱",
        "CompanyName", "SecuritiesCompanyName", "stock_name"
    ]) or code).strip()
    ...
    change = to_float(pick(row, [
        "Change", "漲跌價差", "漲跌", "漲跌(+/-)", "漲跌(+/-)價差",
    ...
    volume = to_float(pick(row, [
        "TradeVolume", "成交股數", "成交股數(股)", "成交量", "Volume", "volume"
    ]))
    amount = to_float(pick(row, [
        "TradeValue", "成交金額", "成交金額(元)", "成交值", "Amount", "amount"
    ]))
...
def ensure_tables() -> None:
    ...
    with get_conn() as conn:
        if conn.postgres:
            for col in ["change DOUBLE PRECISION", "volume DOUBLE PRECISION", "amount DOUBLE PRECISION", "market TEXT", "source TEXT", "trade_date TEXT"]:
                conn.execute(f"ALTER TABLE stock_quotes ADD COLUMN IF NOT EXISTS {col}")
            conn.execute("""
...
def update_official_close_quotes() -> dict[str, Any]:
    ...
    return {"updated_at": now_iso(), "quotes_fetched": len(quotes), "quotes_saved": saved, "source": "official_openapi_close"}
