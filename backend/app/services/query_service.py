...
from ..database import get_conn, init_db, rows_to_dicts, normal_stock_condition
...
def compute_etf_changes(conn, etf_code: str, date: str, prev_date: str | None = None) -> list[dict[str, Any]]:
    ...
    for code, r in curr_map.items():
        ...
        if not p:
            status = "新增"
            delta_shares = r["shares"]
            delta_weight = r["weight"]
        else:
    ...
    for code, p in prev_map.items():
        if (not is_normal_stock_code(code)) or code in curr_map:
            continue
        out.append({**p, "data_date": date, "status": "刪除", "shares": 0, "weight": 0, "delta_shares": -(p["shares"] or 0), "delta_weight": -(p["weight"] or 0)})
...
def get_stock_detail(stock_code: str) -> dict[str, Any]:
    ...
    with get_conn() as conn:
        ...
        hist = rows_to_dicts(conn.execute(
            """
            SELECT * FROM holdings WHERE stock_code=? ORDER BY data_date DESC, etf_code LIMIT 200
            """, (stock_code,)
        ).fetchall())
...
def get_signals(signal_type: str | None = None) -> dict[str, Any]:
    ...
    with get_conn() as conn:
        ...
        by_stock = defaultdict(lambda: {"stock_code":"", "stock_name":"", "delta_shares":0, "delta_weight":0, "count":0, "statuses":[]})
        for c in changes:
            b = by_stock[c["stock_code"]]
            b["stock_code"] = c["stock_code"]
            b["stock_name"] = c["stock_name"]
            b["delta_shares"] += c.get("delta_shares") or 0
            b["delta_weight"] += c.get("delta_weight") or 0
            b["count"] += 1
            b["statuses"].append(f"{c['etf_code']} {c['status']}")
        aggregate = sorted(by_stock.values(), key=lambda x: abs(x["delta_shares"]), reverse=True)
        return {"summary": summarize_changes(changes), "changes": changes, "aggregate": aggregate}
